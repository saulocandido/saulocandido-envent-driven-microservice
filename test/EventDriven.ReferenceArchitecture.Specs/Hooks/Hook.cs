    ...
    public async Task RegisterServices()
    {
        var host = Host
            ...
            .ConfigureServices(services =>
            {
                ...
                var settingsSection = config.GetSection(nameof(ReferenceArchSpecsSettings));
                if (!settingsSection.Exists())
                {
                    throw new Exception($"Configuration section '{nameof(ReferenceArchSpecsSettings)}' not present in app settings.");
                }
                services.Configure<ReferenceArchSpecsSettings>(settingsSection);
                services.AddSingleton(sp => sp.GetRequiredService<IOptions<ReferenceArchSpecsSettings>>().Value);
